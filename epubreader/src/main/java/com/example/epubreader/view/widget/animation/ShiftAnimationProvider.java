...
public final class ShiftAnimationProvider extends SimpleAnimationProvider {
	...
	public ShiftAnimationProvider(PageBitmapManager bitmapManager) {
		super(bitmapManager);
	}
...
//			canvas.save();
//			canvas.clipRect(dX > 0 ? dX - myWidth : dX + myWidth, 0, dX > 0 ? dX  : dX + 2*myWidth, myHeight);
			drawBitmapTo(canvas, dX > 0 ? dX - myWidth : dX + myWidth, 0, myPaint);
//			canvas.restore();

//			canvas.save();
//			canvas.clipRect(dX, 0, dX + myWidth, myHeight);
...
//			final int dX = myEndX - myStartX;
//			if (dX > 0 && dX < myWidth) {
//				canvas.drawBitmap(footerBitmap, -myWidth + dX, voffset, myPaint);
//				canvas.drawBitmap(footerBitmap, dX, voffset, myPaint);
